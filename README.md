...
| **R Analysis Script** | Der vollständige R-Code für Bereinigung, Analyse und Visualisierung. | [Link zur Datei `Skripte_Process.R` in diesem Repo] |
| **Clean Data Export** | Die finale CSV/RDS-Datei der bereinigten Fahrten. | [Link zur Datei `all_trips_clean.rds` oder `summary_user_stats.csv` , `summary_day_of_week_stats.csv` and `summary_top_stations.csv`] |
